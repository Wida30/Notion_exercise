...
const evenNumber = ages1.filter(age => age %2 ==0);
...
const findLetter = streamers1.filter( streamer => streamer.name.includes("u"));
...
const uppper = streamers1.filter((streamer) => 
...
const botonFiltro$$ = document.querySelector(".filter");

const filtrado = ()=> {

	const filter  = streamers2.filter((streamer2) =>
...
botonFiltro$$.addEventListener("input", fitrado)
